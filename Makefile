...
test-os: test-dir all
	@bin/gauss dane/A-os dane/B-os > bin/testy/output-os ;\
	excd=$$? ; \
	if [ $$excd -eq 3 ]; \
	then \
...
test-1: test-dir all
	@bin/gauss dane/d1A dane/d1b > bin/testy/output-t1; \
	excd=$$? ; \
    res=$$(diff -b bin/testy/output-t1 dane/w1) ; \
    if [ -z "$$res" ]; \
    then \
...
test-2: test-dir all
	@bin/gauss dane/d2A dane/d2b > bin/testy/output-t2; \
	excd=$$? ; \
    res=$$(diff -b bin/testy/output-t2 dane/w2) ; \
    if [ -z "$$res" ]; \
    then \
...
test-3: test-dir all
	@bin/gauss dane/d3A dane/d3b > bin/testy/output-t3; \
	excd=$$? ; \
    res=$$(diff -b bin/testy/output-t3 dane/w3) ; \
    if [ -z "$$res" ]; \
    then \
...
test-4: test-dir all
	@bin/gauss dane/d4A dane/d4b > bin/testy/output-t4; \
	excd=$$? ; \
    if [ $$excd -eq 4 ]; \
    then \
